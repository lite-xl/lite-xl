...
graph TD
    subgraph C_Core
        A[src/main.c] -->|SDL init| B[src/renderer.c]
        A -->|Lua state| C[src/api/api.c]
        B -->|Font/Text| D[src/renwindow.c]
        D -->|Surface/Window| E[SDL_Window]
        B -->|Command buffer| F[src/rencache.c]
        F -->|Dirty rects| D
        C -->|Bindings| G[renderer.lua]
        C -->|Bindings| H[system.lua]
        C -->|Bindings| I[process.lua]
        C -->|Bindings| J[regex.lua]
        C -->|Bindings| K[utf8.lua]
        C -->|Bindings| L[dirmonitor.lua]
    end

    subgraph Lua_Core
        M[data/core/start.lua] -->|Bootstrap| N[data/core/init.lua]
        N -->|Require modules| O[data/core/view.lua]
        N -->|Require modules| P[data/core/doc/init.lua]
        N -->|Require modules| Q[data/core/command.lua]
        N -->|Require modules| R[data/core/keymap.lua]
        N -->|Require modules| S[data/core/tokenizer.lua]
        N -->|Require modules| T[data/core/syntax.lua]
        N -->|Require modules| U[data/core/rootview.lua]
        O -->|Base class| V[DocView]
        P -->|Model| V
        V -->|Rendering| G
        Q -->|Registry| R
        R -->|Shortcuts| U
    ...
    subgraph Plugins
        W[data/plugins/autocomplete.lua] -->|Symbols| P
        X[data/plugins/language_*.lua] -->|Syntax| T
        Y[Native Plugin] -->|C API| C
    end

    subgraph User
        Z[User Input] -->|Events| A
        Z -->|Events| U
    end
