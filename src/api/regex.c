...
static pcre2_code* regex_get_pattern(lua_State* L, bool* should_free) {
  ...
  *should_free = false;
  ...
  if (lua_type(L, 1) == LUA_TTABLE) {
    ...
    re = pcre2_compile((PCRE2_SPTR) pattern, pattern_len, PCRE2_UTF, &errornumber, &erroroffset, NULL);

    if (re == NULL) {
      PCRE2_UCHAR errmsg[256];
      pcre2_get_error_message(errornumber, errmsg, sizeof(errmsg));
      luaL_error(L, "regex pattern error at offset %d: %s", (int) erroroffset, errmsg);
      return NULL;
    }
...
static int regex_gmatch_iterator(lua_State* L) {
  ...
  if (state->found) {
    int rc = pcre2_match(state->re, (PCRE2_SPTR) state->subject, state->subject_len, state->offset, 0, state->match_data, NULL);
    ...
    if (rc < 0) {
      ...
      if (ovector_count > 0) {
        ...
        if (ovector[0] > ovector[1]) {
          ...
          luaL_error(
            L,
            "regex matching error: \\K was used in an assertion to "
...
static int f_pcre_compile(lua_State* L) {
  ...
  pcre2_code* re = pcre2_compile((PCRE2_SPTR) str, len, pattern, &errorNumber, &errorOffset, NULL);
  if (re) {
    pcre2_jit_compile(re, PCRE2_JIT_COMPLETE);
    lua_newtable(L);
...
static int f_pcre_gmatch(lua_State* L) {
  ...
  /* offset param */
  size_t offset = regex_offset_relative(luaL_optnumber(L, 3, 1), subject_len) - 1;
