...
static HitTestInfo window_hit_info[1] = {
  { 0, 0, 0 }
};
...
static const char* get_key_name(const SDL_Event* e, char* buf) {
  ...
  ** and SDL_SCANCODE_KP_PERIOD are declared in SDL2 in that order. */
  if (scancode >= SDL_SCANCODE_KP_1 && scancode <= SDL_SCANCODE_KP_1 + 10 && !(e->key.keysym.mod & KMOD_NUM)) {
...
top:
  ...
  switch (e.type) {
    ...
    case SDL_MOUSEWHEEL: lua_pushstring(L, "mousewheel");
    ...
    case SDL_APP_WILLENTERBACKGROUND: lua_pushstring(L, "enteringbackground"); return 1;
    case SDL_APP_DIDENTERBACKGROUND:  lua_pushstring(L, "enteredbackground"); return 1;
...
static const int cursor_enums[] = {
  SDL_SYSTEM_CURSOR_ARROW, SDL_SYSTEM_CURSOR_IBEAM, SDL_SYSTEM_CURSOR_SIZEWE, SDL_SYSTEM_CURSOR_SIZENS, SDL_SYSTEM_CURSOR_HAND
};
...
static int f_set_window_mode(lua_State* L) {
  ...
  SDL_SetWindowFullscreen(window_renderer->window, n == WIN_FULLSCREEN ? SDL_WINDOW_FULLSCREEN_DESKTOP : 0);
  if (n == WIN_NORMAL) {
