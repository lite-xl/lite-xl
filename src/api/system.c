...
extern RenWindow *active_window_renderer;
...
static int f_set_text_input_rect(lua_State *L) {
  ...
  SDL_SetTextInputArea(active_window_renderer->window, &rect, 0);
  return 0;
}

static int f_clear_ime(lua_State *L) {
  SDL_ClearComposition(active_window_renderer->window);
  return 0;
}
...
static int f_text_input(lua_State* L) {
  ...
  {    
    if (lua_toboolean(L, 1)) {
      SDL_StartTextInput(active_window_renderer->window);
    } else {
      SDL_StopTextInput(active_window_renderer->window);
    }
  }
  return 0;
}
