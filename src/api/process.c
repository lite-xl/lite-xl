...
typedef enum { SIGNAL_KILL, SIGNAL_TERM, SIGNAL_INTERRUPT } signal_e;

typedef enum { WAIT_NONE = 0, WAIT_DEADLINE = -1, WAIT_INFINITE = -2 } wait_e;
...
cleanup:
  if (proc_env_block)
    FreeEnvironmentStringsW(proc_env_block);
  return retval;
}
#endif
...
static int process_start(lua_State* L) {
  ...
  if (lua_istable(L, 2)) {
    ...
    deadline = luaL_optnumber(L, -1, deadline);
    lua_getfield(L, 2, "cwd");
    cwd = luaL_optstring(L, -1, NULL);
    lua_getfield(L, 2, "stdin");
    new_fds[STDIN_FD] = luaL_optnumber(L, -1, STDIN_FD);
    lua_getfield(L, 2, "stdout");
    new_fds[STDOUT_FD] = luaL_optnumber(L, -1, STDOUT_FD);
    lua_getfield(L, 2, "stderr");
    new_fds[STDERR_FD] = luaL_optnumber(L, -1, STDERR_FD);
      ...
      default:
        {
          if (new_fds[i] == i) {
            ...
            self->child_pipes[i][0] = CreateNamedPipeA(
              pipeNameBuffer,
            ...
            self->child_pipes[i][1] = CreateFileA(
              pipeNameBuffer, GENERIC_WRITE, 0, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL
            );
  ...
  if (!CreateProcessW(
        ...
        NULL,
        true,
        (detach ? DETACHED_PROCESS : CREATE_NO_WINDOW) | CREATE_UNICODE_ENVIRONMENT,
        env_vars,
        cwd ? cwd_w : NULL,
        &siStartInfo,
...
static int g_read(lua_State* L, int stream, unsigned long read_size) {
  ...
  if (self->reading[writable_stream_idx]
      || !ReadFile(
        self->child_pipes[stream][0],
        self->buffer[writable_stream_idx],
        read_size > READ_BUF_SIZE ? READ_BUF_SIZE : read_size,
        NULL,
        &self->overlapped[writable_stream_idx]
      )) {
...
static int f_terminate(lua_State* L) {
  return self_signal(L, SIGNAL_TERM);
}
static int f_kill(lua_State* L) {
  return self_signal(L, SIGNAL_KILL);
}
static int f_interrupt(lua_State* L) {
  return self_signal(L, SIGNAL_INTERRUPT);
}
