...
static int f_font_load(lua_State *L) {
  ...
  *font = ren_font_load(window_renderer, filename, size, antialiasing, hinting, style, ligopt);
  if (!*font) {
...
static int f_font_copy(lua_State *L) {
  ...
  for (int i = 0; i < FONT_FALLBACK_MAX && fonts[i]; ++i) {
    RenFont** font = lua_newuserdata(L, sizeof(RenFont*));
    *font = ren_font_copy(window_renderer, fonts[i], size, antialiasing, hinting, style, ligopt_ptr);
    if (!*font) {
