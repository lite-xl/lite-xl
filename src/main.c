...
init_lua:
  ...
  const char *init_lite_code =
      ...
      "local _require, _dofile, _loadfile, _list_dir, _get_file_info, _io_open = require, dofile, loadfile, system.list_dir, system.get_file_info, io.open\n"
      ...
      "  return f and load(f, '=' .. path:gsub('%%INTERNAL%%" LITE_PATHSEP_PATTERN "', '')) or _loadfile(path)\n"
      "end\n"
      ...
      "    if contents then\n"
      "      package.loaded[modname] = load(contents, '=' .. path:gsub('%%INTERNAL%%"  LITE_PATHSEP_PATTERN  "', ''))()\n" 
      ...
      "    local contents = system.packaged_file(path)\n"
      "    if contents then\n"
      "      path = (os.getenv('TMPDIR') or '/tmp/') .. common.basename(path)\n"
