...
static inline int rencache_min(int a, int b) {
  return a < b ? a : b;
}
static inline int rencache_max(int a, int b) {
  return a > b ? a : b;
}
...
static void* push_command(RenWindow* window_renderer, enum CommandType type, int size) {
  ...
  while (n > window_renderer->command_buf_size) {
    if (!expand_command_buffer(window_renderer)) {
      fprintf(
        stderr,
        "Warning: (" __FILE__ "): unable to resize command buffer (%zu)\n",
...
double rencache_draw_text(
  RenWindow* window_renderer, RenFont** fonts, const char* text, size_t len, double x, int y, RenColor color
) {
  int x_offset;
...
void rencache_end_frame(RenWindow* window_renderer) {
  ...
  for (int i = 0; i < rect_count; i++) {
    ...
    while (next_command(window_renderer, &cmd)) {
      ...
      switch (cmd->type) {
        case SET_CLIP:  ren_set_clip_rect(window_renderer, intersect_rects(ccmd->rect, r)); break;
        case DRAW_RECT: ren_draw_rect(&rs, rcmd->rect, rcmd->color); break;
        case DRAW_TEXT:
