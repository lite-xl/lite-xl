...
static int font_set_load_options(RenFont* font) {
  int load_target = font->antialiasing == FONT_ANTIALIASING_NONE
                      ? FT_LOAD_TARGET_MONO
...
static unsigned int font_get_glyph_id(RenFont* font, unsigned int codepoint) {
  ...
  if (!font->charmap.rows[row])
    font->charmap.rows[row] = check_alloc(calloc(sizeof(unsigned int), CHARMAP_COL));
...
static SDL_Surface* font_allocate_glyph_surface(RenFont* font, FT_GlyphSlot slot, int bitmap_idx, GlyphMetric* metric) {
  ...
  if (atlas_idx < 0) {
    ...
    for (int i = 0; i < FONT_BITMAP_COUNT(font); i++) {
      ...
      font->glyphs.atlas[i][font->glyphs.natlas] = (GlyphAtlas) {
        .width = metric->x1 + FONT_WIDTH_OVERFLOW_PX,
        .nsurface = 0,
        .surfaces = NULL,
      };
    }
  ...
  if (surface_idx < 0) {
    ...
    font->glyphs
      .bytesize += (sizeof(SDL_Surface*) + sizeof(SDL_Surface) + atlas->width * GLYPHS_PER_ATLAS * h * FONT_BITMAP_COUNT(font));
  }
...
static void font_load_glyph(RenFont* font, unsigned int glyph_id) {
  ...
  if (FT_Load_Glyph(
        font->face, glyph_id, (load_option | FT_LOAD_BITMAP_METRICS_ONLY | FT_LOAD_NO_HINTING) & ~FT_LOAD_FORCE_AUTOHINT
      )
      != 0)
    return;
  ...
  for (int bitmap_idx = 0; bitmap_idx < bitmaps; bitmap_idx++) {
    ...
    if (!slot->bitmap.width || !slot->bitmap.rows || !slot->bitmap.buffer
        || (slot->bitmap.pixel_mode != FT_PIXEL_MODE_MONO && slot->bitmap.pixel_mode != FT_PIXEL_MODE_GRAY && slot->bitmap.pixel_mode != FT_PIXEL_MODE_LCD))
      continue;
...
static RenFont* font_group_get_glyph(
  RenFont** fonts, unsigned int codepoint, int subpixel_idx, SDL_Surface** surface, GlyphMetric** metric
) {
  if (subpixel_idx < 0)
    subpixel_idx += SUBPIXEL_BITMAPS_CACHED;
  RenFont* font = NULL;
  unsigned int glyph_id = 0;
  for (int i = 0; i < FONT_FALLBACK_MAX && fonts[i]; i++) {
    font = fonts[i];
    glyph_id = font_get_glyph_id(fonts[i], codepoint);
  ...
  if (!m || !(m->flags & EGlyphLoaded))
    font_load_glyph(font, glyph_id);
  ...
  if (metric && m)
    *metric = m;
  if (surface && m && m->flags & EGlyphBitmap)
    *surface = font->glyphs.atlas[subpixel_idx][m->atlas_idx].surfaces[m->surface_idx];
  return font;
}
...
static int font_set_face_metrics(RenFont* font, FT_Face face) {
  ...
  if ((err = FT_Load_Char(
         face, ' ', (font_set_load_options(font) | FT_LOAD_BITMAP_METRICS_ONLY | FT_LOAD_NO_HINTING) & ~FT_LOAD_FORCE_AUTOHINT
       ))
      != 0)
    return err;
...
RenFont* ren_font_load(
  const char* path, float size, ERenFontAntialiasing antialiasing, ERenFontHinting hinting, unsigned char style
) {
  SDL_RWops* file = NULL;
  RenFont* font = NULL;
  FT_Face face = NULL;
...
#define FONT_GET_XADVANCE(F, C, M) \
  (is_whitespace((C)) || !(M) || !(M)->xadvance ? (F)->space_advance * (float) ((C) == '\t' ? (F)->tab_size : 1) : (M)->xadvance)
...
double ren_draw_text(RenSurface* rs, RenFont** fonts, const char* text, size_t len, float x, int y, RenColor color) {
  ...
  while (text < end) {
    ...
    RenFont* font = font_group_get_glyph(
      fonts, codepoint, (int) (fmod(pen_x, 1.0) * SUBPIXEL_BITMAPS_CACHED), &font_surface, &metric
    );
    if (!metric)
      break;
    ...
    if (!is_whitespace(codepoint) && font_surface && color.a > 0 && end_x >= clip.x && start_x < clip_end_x) {
      ...
      for (int line = metric->y0; line < metric->y1; ++line) {
        ...
        uint32_t* destination_pixel = (uint32_t*) &(
          destination_pixels[surface->pitch * target_y + start_x * surface->format->BytesPerPixel]
        );
        ...
        for (int x = glyph_start; x < glyph_end; ++x) {
          ...
          SDL_Color dst = { (destination_color & surface->format->Rmask) >> surface->format->Rshift,
          ...
          // the standard way of doing this would be SDL_GetRGBA, but that introduces a performance regression. needs to be investigated
          *destination_pixel++ = (unsigned int) dst.a << surface->format->Ashift | r << surface->format->Rshift
...
void ren_draw_rect(RenSurface* rs, RenRect rect, RenColor color) {
  ...
  SDL_Rect dest_rect = {
    rect.x * surface_scale, rect.y * surface_scale, rect.width * surface_scale, rect.height * surface_scale
  };
  ...
  if (color.a == 0xff) {
    ...
    SDL_Rect clip;
    SDL_GetClipRect(surface, &clip);
    if (!SDL_IntersectRect(&clip, &dest_rect, &dest_rect))
      return;
